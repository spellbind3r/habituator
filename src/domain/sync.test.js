...
test("computeLatestState breaks equal serverTimestamp ties with eventId", () => {
  const events = [
    makeEvent({ eventId: "e7", value: { kind: "binary", value: false } }),
    makeEvent({ eventId: "e9", value: { kind: "binary", value: true } })
  ];

  const [state] = computeLatestState(events);
  assert.equal(state.latestEventId, "e9");
  assert.deepEqual(state.value, { kind: "binary", value: true });
});
